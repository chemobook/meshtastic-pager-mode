...
void applyPagerModeToCurrentThread()
{
    if (!pagerModeEnabled)
        return;

    currentMode = pagerThreadMode;
    currentChannel = pagerChannel;
    currentPeer = pagerPeer;
    didReset = false;
}

void loadPagerModeState()
{
    if (pagerStateLoaded)
        return;
...
bool pagerModeMatchesMessage(const StoredMessage &sm, const meshtastic_MeshPacket &packet)
{
    ...
    if (pagerThreadMode == ThreadMode::CHANNEL) {
        return (packet.to == 0 || packet.to == NODENUM_BROADCAST) && static_cast<int>(sm.channelIndex) == pagerChannel;
    }

    if (pagerThreadMode == ThreadMode::DIRECT) {
...
{
    loadPagerModeState();
    return pagerModeEnabled;
}

void setPagerModeEnabled(bool enabled)
{
    loadPagerModeState();
...
{
    setPagerModeEnabled(!isPagerModeEnabled());
}
...
void buildThreadTitle(char *titleStr, size_t titleSize)
{
    snprintf(titleStr, titleSize, "Messages");
    switch (currentMode) {
    case ThreadMode::ALL:
        snprintf(titleStr, titleSize, "Messages");
        break;
    case ThreadMode::CHANNEL: {
        ...
        if (cname && cname[0]) {
            snprintf(titleStr, titleSize, "#%s", cname);
        } else {
            snprintf(titleStr, titleSize, "Ch%d", currentChannel);
        }
        break;
    }
    case ThreadMode::DIRECT: {
        ...
        if (node && node->has_user && node->user.short_name[0]) {
            snprintf(titleStr, titleSize, "@%s", node->user.short_name);
        } else {
            snprintf(titleStr, titleSize, "@%08x", currentPeer);
        }
        break;
    }
    }
...
void drawPagerFocusedMessage(OLEDDisplay *display, int16_t x, int16_t y, const char *titleStr, const StoredMessage &message)
{
    const char *messageText = MessageStore::getText(message);
    if (!messageText)
        messageText = "";
    ...
    struct PagerFontChoice {
        const uint8_t *font;
        int fontHeight;
    };

    const PagerFontChoice fontChoices[] = {
        {FONT_LARGE, FONT_HEIGHT_LARGE},
        {FONT_MEDIUM, FONT_HEIGHT_MEDIUM},
    };

    std::vector<std::string> wrappedLines;
    int lineHeight = FONT_HEIGHT_MEDIUM;

    for (const auto &choice : fontChoices) {
        display->setFont(choice.font);
        std::vector<std::string> candidate = generateLines(display, "", messageText, availableWidth);
        if (candidate.empty())
            candidate.emplace_back("");

        const int totalHeight = static_cast<int>(candidate.size()) * choice.fontHeight;
        wrappedLines = std::move(candidate);
        lineHeight = choice.fontHeight;

        if (totalHeight <= availableHeight)
            break;
    }

    static uint32_t pagerSignature = 0;
    uint32_t newSignature = 2166136261u;
    const auto mixSignature = [&newSignature](uint32_t value) {
        newSignature ^= value;
        newSignature *= 16777619u;
    };
    mixSignature(static_cast<uint32_t>(currentMode));
    mixSignature(static_cast<uint32_t>(currentChannel));
    mixSignature(currentPeer);
    mixSignature(message.sender);
    mixSignature(message.dest);
    mixSignature(message.timestamp);
    mixSignature(static_cast<uint32_t>(lineHeight));
    ...
    if (totalHeight <= visibleHeight) {
        ...
        if (!scrollStarted && now - scrollStartDelay > 1800)
            scrollStarted = true;
        ...
        if (!manualScrolling) {
            if (scrollStarted) {
                ...
                } else if (lastTime - pauseStart > 2500) {
                    scrollY = 0.0f;
                    waitingToReset = false;
                    scrollStarted = false;
    ...
    const int finalScroll = (totalHeight > visibleHeight) ? static_cast<int>(scrollY) : 0;
    int cursorY = contentTop - finalScroll;
    if (totalHeight < availableHeight)
...
void drawTextMessageFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    char titleStr[48];
    buildThreadTitle(titleStr, sizeof(titleStr));

    if (filtered.empty()) {
        // Outside pager mode we fall back to ALL when the current thread is empty.
        if (currentMode != ThreadMode::ALL && !pagerModeEnabled) {
