...
void InkHUD::ThreadedMessageApplet::onRender(bool full)
{
    ...
    if (channels.isDefaultChannel(channelIndex))
        headerText = "#Public";
    else if (channels.getByIndex(channelIndex).settings.name[0])
        headerText = "#" + std::string(channels.getByIndex(channelIndex).settings.name);
    else
        headerText = "Ch " + to_string(channelIndex);

    if (isPagerModeEnabled()) {
        if (store->messages.empty()) {
            drawHeader(headerText);
            printAt(X(0.5), Y(0.5), "No messages", CENTER, MIDDLE);
            return;
        }

        std::string text = parse(store->messages.front().text);
        drawHeader(headerText);
...
bool InkHUD::ThreadedMessageApplet::isPagerModeEnabled() const
{
    return inkhud->persistence->pager.active &&
           inkhud->persistence->pager.kind == Persistence::PagerState::CHANNEL &&
           inkhud->persistence->pager.channelIndex == channelIndex;
}

void InkHUD::ThreadedMessageApplet::setPagerMode(bool enabled)
{
    auto &pager = inkhud->persistence->pager;
    pager.active = enabled;
    pager.kind = enabled ? Persistence::PagerState::CHANNEL : Persistence::PagerState::NONE;
    pager.channelIndex = enabled ? channelIndex : 0xFF;
