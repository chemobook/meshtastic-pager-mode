...
void InkHUD::MenuApplet::execute(MenuItem item)
{
    ...
    case TOGGLE_PAGER_MODE: {
        Applet *activeApplet = inkhud->getActiveApplet();
        if (activeApplet && activeApplet->supportsPagerMode()) {
            activeApplet->setPagerMode(!activeApplet->isPagerModeEnabled());
...
void InkHUD::MenuApplet::showPage(MenuPage page)
{
    ...
    case ROOT:
        ...
        if (Applet *activeApplet = inkhud->getActiveApplet(); activeApplet && activeApplet->supportsPagerMode()) {
            items.push_back(MenuItem(activeApplet->isPagerModeEnabled() ? "Disable Pager Mode" : "Enable Pager Mode",
                                     MenuAction::TOGGLE_PAGER_MODE, MenuPage::EXIT));
        }
