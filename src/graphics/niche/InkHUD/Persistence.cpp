...
void InkHUD::Persistence::loadPagerState()
{
    PagerState loadedPager;
    bool loadSucceeded = FlashData<PagerState>::load(&loadedPager, "pager");
    if (loadSucceeded && loadedPager.version == PAGER_STATE_VERSION)
        pager = loadedPager;
}
...
void InkHUD::Persistence::savePagerState()
{
    FlashData<PagerState>::save(&pager, "pager");
}
