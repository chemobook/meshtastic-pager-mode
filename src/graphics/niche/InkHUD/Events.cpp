...
inline InkHUD::Applet *activePagerApplet(InkHUD::InkHUD *inkhud)
{
    if (!inkhud)
        return nullptr;

    InkHUD::Applet *applet = inkhud->getActiveApplet();
    if (applet && applet->supportsPagerMode() && applet->isPagerModeEnabled())
        return applet;
...
void InkHUD::Events::onButtonLong()
{
    ...
    if (consumer)
        ...
        if (Applet *pagerApplet = activePagerApplet(inkhud)) {
            pagerApplet->setPagerMode(false);
            return;
        }
...
int InkHUD::Events::onReceiveTextMessage(const meshtastic_MeshPacket *packet)
{
    ...
    if (!isBroadcast(packet->to) && pager.active && pager.kind == Persistence::PagerState::DM && pager.peer != 0 &&
        packet->from != pager.peer)
        return 0;
